...
namespace AddressLibrary.Helpers
{
    ...
    {
        private readonly Dictionary<(string Cecha, string Original), (string Nazwa1, string Nazwa2)> _conversionDict;
        ...
        public StreetNamePersonalConverter(string appDataPath)
        {
            ...
            _conversionDict = new Dictionary<(string, string), (string, string)>();
        ...
        private void LoadFromExcel(string appDataPath)
        {
            ...
            foreach (var row in rows)
            {
                ...
                var cecha = GetCellValue(workbookPart, cells[0]);
                var nazwa1 = GetCellValue(workbookPart, cells[1]);
                var nazwa2 = GetCellValue(workbookPart, cells[2]);
                var original = GetCellValue(workbookPart, cells[3]);
                ...
                if (!string.IsNullOrWhiteSpace(original))
                {
                    ...
                    var value = (nazwa1.Trim(), nazwa2.Trim());
